...
$$
H_0: \mu = 75 \quad \text{(kein Unterschied, Mittelwert entspricht 75)}
$$
...
*Warum 75 als Populationsmittelwert?*

Der Wert $\mu_0 = 75$ ist eine Annahme. In vielen Fällen kommt er aus vorherigen Studien, Erfahrungswerten oder offiziellen Angaben. Falls wir z. B. wissen, dass Studierende in den letzten Jahren im Schnitt 75 Punkte hatten, könnte das unser Referenzwert sein.
...
- Der t-Wert gibt an, wie viele Standardfehler der Stichprobenmittelwert von $\mu_0$ entfernt ist.
...
Wobei $\bar{x}$ der Mittelwert der Stichprobe ist, $\mu_0$ der Mittelwert der Grundgesamtheit ist, $s_x$ der Standardfehler ist und $n$ die Anzahl der Beobachtungen ist.
:::
...
$$
CI_{(1-\alpha)} = \bar{x} \pm t_{\alpha/2, df} \cdot s_{\bar{x}}
$$
...
|-----------------------------------|---------------------------------------------|-----------|
| Stichprobe                        | $x = \{70, 80, 50, 90, 60\}$                |           |
| Stichprobenmittelwert $\bar{x}$   | $\frac{70+80+50+90+60}{5}$                  | **70.00** |
| Stichprobenstandardabweichung $s$ | $\sqrt{\frac{\sum (x_i - \bar{x})^2}{n-1}}$ | **15.81** |
| Standardfehler $s_{\bar{x}}$      | $\frac{s}{\sqrt{n}}$                        | **7.07**  |
